...
<form on:submit={handleSubmit} class="space-y-6">
	...
	<div>
		...
		<input
			...
			class="w-full rounded-lg border border-gray-300 px-4 py-2 transition-colors focus:right-2 focus:border-transparent focus:ring-blue-500 disabled:cursor-not-allowed disabled:bg-gray-100"
			placeholder="you@example.com"
			autocomplete="email"
			required
		/>
		{#if emailError}
			<p class="test-sm mt-1 text-red-600">{emailError}</p>
		{/if}
	</div>
