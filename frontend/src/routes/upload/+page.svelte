...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="text-center mb-8">
        ...
        <p class="text-lg text-gray-600">
            Upload a PDF file to start text-to-speech processing
        </p>
    </div>
    ...
    <div class="card">
        <form on:submit={handleUpload} class="space-y-6">
            <div>
                ...
                <div
                    class="rounded-lg border-2 border-dashed p-6 text-center transition-colors {isDragActive ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-gray-50'}"
                    role="region"
                ...
                >
                    <p class="text-sm text-gray-700">
                        Drag and drop a PDF here, or use the picker below.
                    </p>
                </div>
                <input
                    bind:this={fileInput}
            ...
            {#if uploadSuccess}
                <div class="rounded-lg border border-green-200 bg-green-50 p-4">
                    ...
                    {#if uploadedFile}
                        <p class="text-sm text-green-700 mt-1">Status: {uploadedFile.status}</p>
                        <p class="text-sm text-green-700 mt-1">Uploaded: {new Date(uploadedFile.upload_date).toLocaleString()}</p>
                    {/if}
                </div>
            {/if}
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <Button type="submit" loading={isUploading} loadingText="Uploading..." disabled={!selectedFile}>
                    Upload PDF
                </Button>
            </div>
        </form>
